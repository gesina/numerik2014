...
\begin{Bspe}
  ...
  \item $x=1+e^{-x} \eqqcolon g_1(x)$
    ...
    \end{tikzpicture}\\
    {\centering 
      \textit{Konvergenz der Fixpunktiteration für $x=1+e^{-x}$}
    }
    \label{im5.2.1(1)}
  ...
  \item $e^{-x} = x-1   \Leftrightarrow x= -ln(x-1) \eqqcolon g_2(x)$\\
    ...
    \end{tikzpicture}\\
    {\centering 
      \textit{Versagen der Fixpunktiteration für $x=-ln(x-1)$}
    }
    \label{im5.2.1(2)}
...
\end{tikzpicture}\\
{\centering
  \textit{Newton-Verfahren bei einem Funktionsgraphen}
}
\label{im5.4}
...
\begin{Bspe}[Newton-Verfahren ohne Konvergenz]~
  ...
  \item $x^{(1)}$ nicht mehr im Definitionsbereich\\
    ...
    \end{tikzpicture}
    {\centering
      \textit{Fehlschlagen des Newton-Verfahrens: außerhalb des
        Definitionsbereichs}
    }
    \label{im5.4.4(1)}
  ...
  \item $|x^{*}-x^{(1)}| \nless |x^{*}-x^{(0)}| $
    ...
    \end{tikzpicture}\\
    {\centering
      \textit{Fehlschlagen des Newton-Verfahren: Konvergenz nicht
        gesichert}
    }
    \label{im5.4.4(2)}
  \end{itemize}
...
\end{tikzpicture}\\
{\centering \textit{Geometrische Veranschaulichung des
    Sekantenverfahrens}
}
\label{im5.4.7}
