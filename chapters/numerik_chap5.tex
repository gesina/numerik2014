...
\imagemissing{Beispiel zur Nullstellenexistenz}\label{im5.1.2}
...
\begin{Bspe}
	...
	\begin{enumerate}[a)]
		...
		\imagemissing{Konvergenz der Fixpunktiteration für $x=1+e^{-x}$}\label{im5.2.1(1)}
		$\longrightarrow$ Konvergenz
		\item $e^{-x} = x-1   \Leftrightarrow x= -ln(x-1) \eqqcolon g_2(x)$
		\imagemissing{Versagen der Fixpunktiteration für $x=-ln(x-1)$}\label{im5.2.1(2)}
...
\begin{Defe}
	...
	\imagemissing{Grafische Veranschaulichung einer Kontraktion}\label{im5.2.2}
...
\imagemissing{Veranschaulichung des Newton-Verfahrens an einem Funktionsgraphen}\label{im5.4}
...
\begin{Bspe}[Newton-Verfahren ohne Konvergenz]~
	...
	\begin{itemize}
		...
		\imagemissing{Fehlschlagen des Newton-Verfahren: außerhalb des Definitionsbereichs}\label{im5.4.4(1)}
		\item $|x^{*}-x^{(1)}| \nless |x^{*}-x^{(0)}| $
		\imagemissing{Fehlschlagen des Newton-Verfahren: Konvergenz nicht gesichert}\label{im5.4.4(2)}
	\end{itemize}
\end{Bspe}
...
\imagemissing{Geometrische Veranschaulichung des Sekantenverfahrens}\label{im5.4.7}
