...
\newcommand{\Rnm}{\mathds{R}^{n\times m}}
...
Die Gauß-Elimination mit Spaltenpivotsuche

MISSING

\sectione{Cholesky-Verfahren}
Im Folgenden sei $A$ eine symmetrische, positiv definite Matrix in $\Rnn $, d.h.
$A=A^T$ und $\langle x, Ax \rangle = x^TAx > 0 \forall x\neq 0$. \\
(kurs: spd Matrix)

\subsection{Satz} \label{4.2.1}
Für jede spd Matrix $A\in \Rnn $ gilt:
\begin{enumerate}[i)]	
...
\begin{enumerate}[i)]
	...
	\item Es gilt:
		...
		Diese Zerlegung heißt \textbf{rationale Cholesky-Zerlegung}\index{Choleskyzerlegung!rational}. Die Zerlegung
		\begin{gather}
			A= \bar{L}\bar{L}^T 
%			= 
%			\begin{pmatrix}
...
\label{IV.2.2}
		...
		\begin{align}
			a_{kk} &= \bar1{l}_{k1}^{2} + \cdots +  \bar{l}_{kk}^2  \label{IV.2.3} \\
	...
	Es sind je 
		...
		\end{enumerate}
		also ca. $\frac{1}{3}$ flops für große $n$ notwendig. \\
		...
		\begin{enumerate}[a)]
			...
			\item Für spd Matrizen ist der Cholesky-Algorithmus stabil nach \ref{MISSING}
