...
\usepackage[backend=biber,style=alphabetic]{biblatex}	%enables to print bibliography with \printbibliography, needs \bibliography{bib ﬁles}
...
% EVIL HACK to go along with equation numeration
\newcommand{\sectione}[1]{\section{#1} \setcounter{equation}{0}}

% EVIL HACK to go along with theorem numeration
...
% EVIL HACK: Satz redefinition to go along with numeration
...
\subsectione{Dreieckszerlegung} \label{2.1.5}
...
\subsectione{Vorwärtssubstitution} \index{Vorwärtssubstitution}
...
\subsectione{Gauß-Elemination zur Lösung von $Ax=b$}\index{Gauß-Eleminator}
\begin{framed}
  \begin{enumerate}[1]
...
\subsectione{Formalisieren des Gauß-Algorithmus} \index{Gaußsches Eliminationsverfahren} \index{LR-Zerlegung}
\begin{enumerate}[a)]
...
\subsectione{Spaltenpivotisierung (=partielle/ halbmaximale Pivotisierung)} \index{Pivotisierung!Spalten-}\index{Pivotisierung}\index{Pivotisierung!halbmaximale}\index{Pivotisierung!partielle}
...
\subsectione{Gauß-Elimination mit Spaltenpivotisierung}\label{2.2.3}
...
%\subsectione{Satz: Dreieckszerlegung mit Permutationsmatrix} 
\begin{Satze}[Dreieckszerlegung mit Permutationsmatri] \label{2.2.4} 
...
%  		\includegraphics[width=1.5cm]{images/normen.png}
                \begin{tikzpicture}[scale=4, line cap=round, x=1.0cm,y=1.0cm]
                    \draw (-2,0) -> (2,0);
                    \draw (0,-2) -> (0,2);
                    \draw [color=rgb(255,0,0)] (-1,-1) rectangle (1,1);
                \end{tikzpicture}
 		\caption{Sphären mit gleichem Normbetrag (Bildquelle: Wikipedia)}
  	\end{figure}
  	\footnotetext{Bildquelle der Sphärenabb.: https://commons.wikimedia.org/wiki/File:Vector\_norms.svg}
